...
func (p *PostModel) List(ctx context.Context, page int, model template.Model) (string, error) {
	...
	// conten, ok := postPage.Content.([]*vo.Post)
	// t := reflect.TypeOf(postPage.Content)
	// fmt.Println("struct name: ", t.Name())
	// fmt.Println("struct info: ", t.String())
	// fmt.Println("postPage:", *conten[0], ok)
	// fmt.Println("postPage Thumbnail:", *&conten[0].Thumbnail == "")
